...
@Component({
  selector: 'app-producto-form-modal-admin',
...
export class ProductoFormModalAdminComponent implements OnInit {
  ...
  guardarProducto() {
    if (!this.nuevoProducto.Nombre_Producto || !this.nuevoProducto.Nombre_Producto.trim()) {
    ...
    const body = {
      ...
      Stock_Producto: Number(this.nuevoProducto.Stock_Producto) || 0,
      Valor_Producto: Number(this.nuevoProducto.Valor_Producto) || 0,
      Unidad_Medida: this.nuevoProducto.Unidad_Medida?.trim() || null,
    };
    ...
    if (this.isEdit && this.nuevoProducto.Id_Producto) {
      ...
      this.http.post<any>(this.apiUrl, body, { headers }).subscribe({
        ...
        error: (error) => {
          ...
          this.mostrarToast(error.error?.message || 'Error guardando el producto', 'danger');
        }
      });
    }
  }
